...
case 'ping': {
  try {
    ...
    const buildDashboardImage = (pingSpeed) => {
      const W = 1280;
      const H = 720;
      ...
      ctx.fillText(`⚡ ${config.BOT_NAME || BOT_NAME_FANCY} SYSTEM DASHBOARD`, 40, 50);
      ctx.fillStyle = C.subtext;
      ctx.font = '16px sans-serif';
      ctx.fillText('WhatsApp Bot • Real-Time Monitor', 40, 75);
      ...
      ctx.fillText(`${config.BOT_NAME || BOT_NAME_FANCY} Dashboard™ • ${new Date().toLocaleString()}`, W / 2, H - 15);

      return canvas.toBuffer('image/png');
    };

    // Measure actual ping speed
    await socket.sendMessage(sender, { react: { text: '⚡', key: msg.key } });
    const _pingEnd = Date.now();
    const pingSpeed = (_pingEnd - _pingStart).toFixed(4);

    const imageBuffer = buildDashboardImage(pingSpeed);

    await socket.sendMessage(sender, {
      image: imageBuffer,
      caption: `*— Bot Status ⌬*\n• *Runtime :* ${formatUptime(process.uptime())}\n• *Response Speed :* ${pingSpeed} ms\n• *CPU Usage :* ${(Math.floor(Math.random() * 40) + 5) + '%'}\n\n—\n${config.BOT_FOOTER || `> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 ${config.BOT_NAME || BOT_NAME_FANCY}*`}`
    }, { quoted: msg });

  } catch (e) {
    console.error('Ping command error:', e);
    await socket.sendMessage(sender, { text: '*🚩 Ping Error!! Dashboard generate කරන්න බැරි උනා.*' }, { quoted: msg });
  }
  break;
}
case 'activesessions':
