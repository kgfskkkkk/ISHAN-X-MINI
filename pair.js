...
case 'mp4': {
    try {
        ...
        if (isYtUrl) {
            ...
            let videoBuffer1;
            try {
                const vRes = await axios.get(json.download.link, { responseType: 'arraybuffer', timeout: 120000 });
                videoBuffer1 = Buffer.from(vRes.data);
            } catch (dlErr) {
                console.error('Video buffer download failed (url-detect):', dlErr.message);
                return await socket.sendMessage(sender, { text: '*📛 Video download කළ නොහැකි විය. නැවත උත්සාහ කරන්න.*' }, { quoted: msg });
            }
            await socket.sendMessage(sender, {
                video: videoBuffer1,
                mimetype: 'video/mp4',
...
case 'down_1080': {
    try {
        ...
        let videoBuffer2;
        try {
            const vRes2 = await axios.get(json.download.link, { responseType: 'arraybuffer', timeout: 120000 });
            videoBuffer2 = Buffer.from(vRes2.data);
        } catch (dlErr2) {
            console.error('Video buffer download failed (down_quality):', dlErr2.message);
            return await socket.sendMessage(sender, { text: '*📛 Video download කළ නොහැකි විය. නැවත උත්සාහ කරන්න.*' }, { quoted: msg });
        }
...
case 'down_1080d': {
    try {
        ...
        let docBuffer;
        try {
            const vRes3 = await axios.get(json.download.link, { responseType: 'arraybuffer', timeout: 120000 });
            docBuffer = Buffer.from(vRes3.data);
        } catch (dlErr3) {
            console.error('Video buffer download failed (doc):', dlErr3.message);
            return await socket.sendMessage(sender, { text: '*📛 Video download කළ නොහැකි විය. නැවත උත්සාහ කරන්න.*' }, { quoted: msg });
        }
