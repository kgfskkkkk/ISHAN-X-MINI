...
case 'mp4': {
    try {
        ...
        if (!q) return await socket.sendMessage(sender, {
            text: `*📛 YouTube Download Command*\n\n` +
                  `*🔗 URL දිලා download:*\n` +
                  `\`${prefix}yt https://youtube.com/watch?v=...\`\n\n` +
                  `*🔍 Keywords දිලා search කරලා download:*\n` +
                  `\`${prefix}yt Alan Walker Faded\`\n\n` +
                  `> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`
        }, { quoted: msg });

        // ✅ YouTube URL detect කළොත් → auto 720p download
        const isYtUrl = /^(https?:\/\/)?(www\.)?(youtube\.com\/(watch\?v=|shorts\/|embed\/)|youtu\.be\/)[\w\-]+/.test(q);
        if (isYtUrl) {
            await socket.sendMessage(sender, { react: { text: '⬇️', key: msg.key } });
            await socket.sendMessage(sender, { text: '*⏳ URL එකෙන් video download කරමින්...*' }, { quoted: msg });
        ...
        // 🔍 Keywords දිලා search කරලා → quality selection menu
        await socket.sendMessage(sender, { react: { text: '🔍', key: msg.key } });
        await socket.sendMessage(sender, { text: `*🔍 "${q}" සොයමින්...*` }, { quoted: msg });
        ...
        const caption =
            ...
            `*┃➤ 🔗 Link     :* ${video.url}\n` +
            `*┗━━━━━━━━━━━━━━━━❖●►*\n\n` +
            `*📥 Quality Format එකක් select කරන්න:*`;
