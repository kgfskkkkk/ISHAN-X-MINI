...
END:VCARD` 
    ...
    // ── Classic Button Message (same style used by the SYSTEM command) ──
    await socket.sendMessage(safeSender, {
        image: imagePayload,
        caption: text,
        contextInfo: {
...
`.trim();
    ...
    // 4. Define the Button System (Classic Buttons — same style used by the SYSTEM command)
    const buttons = [
      { buttonId: `${config.PREFIX || '.'}menu`, buttonText: { displayText: '❄ MAIN MENU' }, type: 1 },
      { buttonId: `${config.PREFIX || '.'}system`, buttonText: { displayText: '❄ SYSTEM' }, type: 1 }
    ];

    // 5. Send the Message (Classic Image + Caption + Buttons)
    await socket.sendMessage(sender, {
      ...
      contextInfo: {
        ...
        externalAdReply: {
          ...
          body: "Tap a button below to interact 👇",
