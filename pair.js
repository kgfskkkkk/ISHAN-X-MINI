...
case 'mp4': {
    try {
        ...
        // Detect YouTube URL (supports watch?v=, youtu.be/, shorts/ formats)
        const ytUrlRegex = /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:watch\?v=|shorts\/)|youtu\.be\/)([\w-]{11})/;
        const ytUrlMatch = q.match(ytUrlRegex);

        let video;

        if (ytUrlMatch) {
            // YouTube URL ලිංකුවක් - video ID extract කරලා directly search කරනවා
            const videoId = ytUrlMatch[1];
            const videoResult = await ytsv({ videoId });
            if (!videoResult || !videoResult.title) {
                return await socket.sendMessage(sender, { text: '*📛 මට කිසිවක් සොයාගත නොහැකි විය :(*' }, { quoted: msg });
            }
            video = videoResult;
            if (!video.url) video.url = `https://www.youtube.com/watch?v=${videoId}`;
            if (!video.views) video.views = 0;
        } else {
            // Search term - සාමාන්‍ය search
