...
// ---------------- YOUTUBE DOWNLOADER (Zanta API) ----------------
const ZANTA_API_KEY = process.env.ZANTA_API_KEY || 'zanta_7ro9KBaXFR4ZQz6eyNSQsD5P';
...
case 'mp4': {
    try {
        ...
        if (isYtUrl) {
            ...
            const quality = '720';
            try {
                const apiUrl = `https://api.zanta-mini.store/api/ytdl?apiKey=${ZANTA_API_KEY}&url=${encodeURIComponent(q)}&type=mp4&quality=${quality}`;
                const res = await axios.get(apiUrl, { timeout: 25000 });
                const rd = res.data;
                if (rd?.success && rd?.result?.download_url) {
                    json = { status: true, title: rd.result.title || 'Video', download: { link: rd.result.download_url, label: rd.result.quality || `${quality}p` } };
                }
            } catch (e1) { console.error('Zanta API failed (url-detect):', e1.message); }
    ...
    try {
        ...
        try {
            ...
            if (rd?.success && rd?.result?.download_url) {
                json = {
                    status: true,
                    title: rd.result.title || 'Video',
                    download: { link: rd.result.download_url, label: rd.result.quality || `${quality}p` }
                };
            }
        } catch (e1) {
            console.error('Zanta API failed (video):', e1.message);
        }
...
case 'down_1080d': {
    try {
        ...
        try {
            ...
            const apiUrl = `https://api.zanta-mini.store/api/ytdl?apiKey=${ZANTA_API_KEY}&url=${encodeURIComponent(q)}&type=mp4&quality=${qParam}`;
            const res = await axios.get(apiUrl, { timeout: 25000 });
            const rd = res.data;
            if (rd?.success && rd?.result?.download_url) {
                json = {
                    status: true,
                    title: rd.result.title || 'Video',
                    thumbnail: rd.result.thumbnail || null,
                    download: { link: rd.result.download_url, label: rd.result.quality || `${qParam}p` }
                };
            }
        } catch (e1) {
            console.error('Zanta API failed (doc):', e1.message);
        }
