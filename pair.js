...
case 'mp4': {
    try {
        ...
        // Detect if the input is a YouTube URL and look up by video ID directly
        const ytUrlRegex = /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/;
        const ytMatch = q.match(ytUrlRegex);

        let video;
        if (ytMatch) {
            const videoId = ytMatch[1];
            const videoInfo = await ytsv({ videoId });
            if (!videoInfo || !videoInfo.title) return await socket.sendMessage(sender, { text: '*📛 මට කිසිවක් සොයාගත නොහැකි විය :(*' }, { quoted: msg });
            video = videoInfo;
        } else {
