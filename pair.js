...
case 'mp4': {
    try {
        ...
        // Detect if user gave a YouTube URL directly (including Shorts)
        const ytUrlRegex = /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:shorts\/|(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=))|youtu\.be\/)([a-zA-Z0-9_-]{11})/;
        const ytUrlMatch = q.match(ytUrlRegex);

        let video;
        if (ytUrlMatch) {
            // URL provided — get video info directly via yt-search
            const videoId = ytUrlMatch[1];
            const infoResult = await ytsv({ videoId });
            if (!infoResult || !infoResult.title) {
                return await socket.sendMessage(sender, { text: '*📛 මට කිසිවක් සොයාගත නොහැකි විය :(*' }, { quoted: msg });
            }
            video = {
                title: infoResult.title,
                author: { name: infoResult.author?.name || 'Unknown' },
                timestamp: infoResult.timestamp || infoResult.duration?.timestamp || 'N/A',
                views: infoResult.views || 0,
                url: `https://www.youtube.com/watch?v=${videoId}`,
                thumbnail: infoResult.thumbnail || `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`
            };
        } else {
