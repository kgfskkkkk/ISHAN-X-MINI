...
case 'mp4': {
    try {
        ...
        if (isYtUrl) {
            ...
            let json = null;
            try {
                const apiUrl = `https://arslan-apis-v2.vercel.app/download/ytmp4?url=${encodeURIComponent(q)}`;
                const res = await axios.get(apiUrl, { timeout: 60000 });
                if (res.data?.status && res.data?.result?.download?.url) {
                    json = {
                        status: true,
                        title: res.data.result.metadata?.title || 'Video',
                        download: { link: res.data.result.download.url, label: '720p' }
                    };
                }
            } catch (e1) { console.error('ArslanMD API failed (url-detect):', e1.message); }
        ...
        try {
            ...
            if (res.data?.status && res.data?.result?.download?.url) {
                json = {
                    status: true,
                    title: res.data.result.metadata?.title || 'Video',
                    download: { link: res.data.result.download.url, label: `${quality}p` }
                };
            }
        } catch (e1) {
            console.error('ArslanMD API failed (video):', e1.message);
        }
...
case 'down_1080d': {
    try {
        ...
        try {
            ...
            const res = await axios.get(apiUrl, { timeout: 60000 });
            if (res.data?.status && res.data?.result?.download?.url) {
                const qLabel = quality === 'best' ? '1080' : quality;
                json = {
                    status: true,
                    title: res.data.result.metadata?.title || 'Video',
                    thumbnail: res.data.result.metadata?.thumbnail || null,
                    download: { link: res.data.result.download.url, label: `${qLabel}p` }
                };
            }
        } catch (e1) {
            console.error('ArslanMD API failed (doc):', e1.message);
        }
...
case 'ytaa': {
    try {
        ...
        const apiUrl = `https://arslan-apis-v2.vercel.app/download/ytmp3?url=${encodeURIComponent(q)}`;
        const res = await axios.get(apiUrl, { timeout: 60000 });
        if (!res.data?.status || !res.data?.result?.download?.url) throw new Error('No download URL returned');
...
case 'ytaap': {
    try {
        ...
        const res = await axios.get(apiUrl, { timeout: 60000 });
        if (!res.data?.status || !res.data?.result?.download?.url) throw new Error('No download URL returned');
        const result = { url: res.data.result.download.url };
...
case 'ytad': {
    try {
        ...
        const ytApiUrl = `https://arslan-apis-v2.vercel.app/download/ytmp3?url=${encodeURIComponent(url)}`;
        const ytApiRes = await axios.get(ytApiUrl, { timeout: 60000 });
        if (!ytApiRes.data?.status || !ytApiRes.data?.result?.download?.url) throw new Error('No download URL returned');
        const result = { url: ytApiRes.data.result.download.url };
