...
END:VCARD` 
    ...
    // =================================================================
    // 🔘 4. INTERACTIVE MESSAGE (nativeFlowMessage style)
    // =================================================================
    const { generateWAMessageFromContent: genPing, prepareWAMessageMedia: prepPing } = require('@dnuzi/baileys');

    let pingHeaderMedia = null;
    try {
        pingHeaderMedia = await prepPing(
            { image: imagePayload },
            { upload: socket.waUploadToServer }
        );
    } catch (e) {
        console.log("Ping header image prepare failed:", e?.message);
    }
    ...
    const pingMsg = {
        viewOnceMessage: {
            message: {
                interactiveMessage: {
                    body: { text: text },
                    footer: { text: `〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰` },
                    header: pingHeaderMedia
                        ? { ...pingHeaderMedia, hasMediaAttachment: true }
                        : { title: botName, subtitle: `〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰`, hasMediaAttachment: false },
                    contextInfo: {
                        externalAdReply: {
                            title: "🚀 ꜱᴘᴇᴇᴅ ᴛᴇꜱᴛ ᴄᴏᴍᴘʟᴇᴛᴇᴅ",
                            body: `Latency: ${finalLatency}ms - Optimized`,
                            thumbnailUrl: String(logo),
    ...
    const pingContent = genPing(sender, pingMsg, { userJid: sender, quoted: metaQuote });
    await socket.relayMessage(sender, pingContent.message, { messageId: pingContent.key.id });
...
`;
    ...
    // ── Interactive Message (nativeFlowMessage) style ──
    const { generateWAMessageFromContent: genSys1, prepareWAMessageMedia: prepSys1 } = require('@dnuzi/baileys');

    let sys1HeaderMedia = null;
    try {
        sys1HeaderMedia = await prepSys1(
            { image: imagePayload },
            { upload: socket.waUploadToServer }
        );
    } catch (e) {
        console.log("System1 header image prepare failed:", e?.message);
    }
    ...
    const sys1Msg = {
        viewOnceMessage: {
            message: {
                interactiveMessage: {
                    body: { text: text },
                    footer: { text: `𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝐒ʏꜱᴛᴇᴍ 𝐈ɴꜰᴏ` },
                    header: sys1HeaderMedia
                        ? { ...sys1HeaderMedia, hasMediaAttachment: true }
                        : { title: botName, subtitle: `𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 System`, hasMediaAttachment: false },
                    contextInfo: {
                        externalAdReply: {
    ...
    const sys1Content = genSys1(sender, sys1Msg, { userJid: sender, quoted: metaQuote });
    await socket.relayMessage(sender, sys1Content.message, { messageId: sys1Content.key.id });
...
`.trim();
    ...
    // ── Interactive Message (nativeFlowMessage) style ──
    const { generateWAMessageFromContent: genSys2, prepareWAMessageMedia: prepSys2 } = require('@dnuzi/baileys');

    let sys2HeaderMedia = null;
    try {
        sys2HeaderMedia = await prepSys2(
            { image: { url: randomLogo } },
            { upload: socket.waUploadToServer }
        );
    } catch (e) {
        console.log("System2 header image prepare failed:", e?.message);
    }
    ...
    const sys2Msg = {
        viewOnceMessage: {
            message: {
                interactiveMessage: {
                    body: { text: text },
                    footer: { text: `⚙️ ꜱʏꜱᴛᴇᴍ ᴄᴏᴍᴍᴀɴᴅꜱ` },
                    header: sys2HeaderMedia
                        ? { ...sys2HeaderMedia, hasMediaAttachment: true }
                        : { title: title, subtitle: `⚙️ 𝐒𝐘𝐒𝐓𝐄𝐌 𝐂𝐎𝐍𝐓𝐑𝐎𝐋`, hasMediaAttachment: false },
                    contextInfo: {
                        externalAdReply: {
    ...
    const sys2Content = genSys2(sender, sys2Msg, { userJid: sender, quoted: shonux });
    await socket.relayMessage(sender, sys2Content.message, { messageId: sys2Content.key.id });
