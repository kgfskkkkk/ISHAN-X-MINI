...
async function ytDownloadHelper(url, quality = '720') {
    let json = null;

    // API 1: ArslanMD API
    if (!json) {
        try {
            const res = await axios.get(`https://arslan-apis-v2.vercel.app/download/ytmp4?url=${encodeURIComponent(url)}`, { timeout: 30000 });
            const d = res.data;
            if (d?.status && d?.result?.download?.url) {
                json = { status: true, title: d.result.metadata?.title || 'Video', thumbnail: d.result.metadata?.thumbnail || null, download: { link: d.result.download.url, label: quality + 'p' } };
            }
        } catch(e) { console.error('ytDL API1 fail:', e.message); }
    }

    // API 2: saviyakolla API
    if (!json) {
        try {
            const res = await axios.get(`https://saviya-kolla-api.koyeb.app/download/youtube?url=${encodeURIComponent(url)}&quality=${quality}`, { timeout: 30000 });
            const d = res.data;
            if (d?.status && d?.result?.download_url) {
                json = { status: true, title: d.result.title || 'Video', thumbnail: d.result.thumbnail || null, download: { link: d.result.download_url, label: quality + 'p' } };
            }
        } catch(e) { console.error('ytDL API2 fail:', e.message); }
    }

    // API 3: giftedtech API
    if (!json) {
        try {
            const qParam = quality === 'best' ? '1080' : quality;
            const res = await axios.get(`https://api.giftedtech.web.id/api/download/ytmp4?apikey=gifted&url=${encodeURIComponent(url)}&quality=${qParam}`, { timeout: 30000 });
            const d = res.data;
            if (d?.success && d?.result?.download_url) {
                json = { status: true, title: d.result.title || 'Video', thumbnail: d.result.thumbnail || null, download: { link: d.result.download_url, label: qParam + 'p' } };
            }
        } catch(e) { console.error('ytDL API3 fail:', e.message); }
    }

    // API 4: dark-yasiya API
    if (!json) {
        try {
            const res = await axios.get(`https://www.dark-yasiya-api.site/download/ytmp4?url=${encodeURIComponent(url)}`, { timeout: 30000 });
            const d = res.data;
            if (d?.status && d?.result?.dl_link) {
                json = { status: true, title: d.result.title || 'Video', thumbnail: d.result.thumb || null, download: { link: d.result.dl_link, label: quality + 'p' } };
            }
        } catch(e) { console.error('ytDL API4 fail:', e.message); }
    }

    // API 5: allstars API
    if (!json) {
        try {
            const res = await axios.get(`https://allstars-apis.vercel.app/ytdl?url=${encodeURIComponent(url)}&quality=${quality}`, { timeout: 30000 });
            const d = res.data;
            const link = d?.data?.download_url || d?.download_url || d?.result?.url || d?.url;
            if (link) {
                json = { status: true, title: d?.data?.title || d?.title || 'Video', thumbnail: d?.data?.thumbnail || null, download: { link, label: quality + 'p' } };
            }
        } catch(e) { console.error('ytDL API5 fail:', e.message); }
    }

    return json;
}
// ==================== END YOUTUBE DOWNLOAD HELPER ====================
...
case 'mp4': {
    try {
        ...
        if (isYtUrl) {
            ...
            const json = await ytDownloadHelper(q, '720');
            if (!json || !json.download?.link) {
                return await socket.sendMessage(sender, { text: '*📛 Video download කළ නොහැකි විය. APIs timeout. නැවත උත්සාහ කරන්න.*' }, { quoted: msg });
            }
            await socket.sendMessage(sender, {
...
case 'down_1080': {
    try {
        ...
        const json = await ytDownloadHelper(q, quality);

        if (!json || !json.download?.link) {
            return await socket.sendMessage(sender, {
                text: '*📛 Video download කළ නොහැකි විය. APIs timeout. නැවත උත්සාහ කරන්න.*'
            }, { quoted: msg });
        }
...
case 'down_1080d': {
    try {
        ...
        const qualityMap = { down_360d: '360', down_480d: '480', down_720d: '720', down_1080d: '1080' };
        const quality = qualityMap[command] || '360';
