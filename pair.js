...
// nntech-free-insta-api.vercel.app doesn't publish a fixed response schema,
// so this digs through the common shapes it (and similar "free API" hubs)
// tend to return and pulls out the first usable media url + its type.
function extractInstaMedia(apiRes) {
  if (!apiRes) return null;

  const candidates = [];
  const pushIfObj = (v) => { if (v && typeof v === 'object') candidates.push(v); };

  if (Array.isArray(apiRes)) candidates.push(...apiRes.filter(v => v && typeof v === 'object'));
  pushIfObj(apiRes.result);
  if (Array.isArray(apiRes.result)) candidates.push(...apiRes.result.filter(v => v && typeof v === 'object'));
  pushIfObj(apiRes.data);
  if (Array.isArray(apiRes.data)) candidates.push(...apiRes.data.filter(v => v && typeof v === 'object'));
  if (Array.isArray(apiRes.medias)) candidates.push(...apiRes.medias.filter(v => v && typeof v === 'object'));
  if (Array.isArray(apiRes.media)) candidates.push(...apiRes.media.filter(v => v && typeof v === 'object'));
  if (Array.isArray(apiRes.links)) candidates.push(...apiRes.links.filter(v => v && typeof v === 'object'));
  candidates.push(apiRes); // root-level keys as last resort

  const urlKeys = ['url', 'video', 'video_url', 'videoUrl', 'download_url', 'downloadUrl', 'hd', 'sd', 'image', 'image_url', 'imageUrl', 'link', 'media_url', 'mediaUrl'];

  for (const item of candidates) {
    for (const key of urlKeys) {
      const val = item[key];
      if (typeof val === 'string' && /^https?:\/\//i.test(val)) {
        const looksLikeImage = /\.(jpe?g|png|webp)(\?|$)/i.test(val) || /image/i.test(key);
        const declaredType = (item.type || item.media_type || '').toString().toLowerCase();
        const type = declaredType.includes('image') || declaredType.includes('photo')
          ? 'image'
          : (declaredType.includes('video') ? 'video' : (looksLikeImage ? 'image' : 'video'));
        return { url: val, type };
      }
    }
  }
...
// ==================== END HELP CENTER ====================
  ...
  if (!jid) {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || '𝐃𝙴𝚅𝙴𝙻𝙾𝙿𝙴𝚁 𝐁𝚈 𝐈𝚂𝙷𝙰𝙽-𝐗 🧑‍💻🇱🇰';
...
END:VCARD`
    ...
    // API request — nntech-free-insta-api (https://nntech-free-api-web.vercel.app)
    let apiUrl = `https://nntech-free-insta-api.vercel.app/?url=${encodeURIComponent(q)}`;
    let { data: apiRes } = await axios.get(apiUrl, { timeout: 30000 }).catch(() => ({ data: null }));

    // Normalize response -> { url, type }
    const media = extractInstaMedia(apiRes);
    ...
    const content = `┏━━━━━━━━━━━━━━━━\n` +
                    `┃📌 \`𝐒ource\` : Instagram\n` +
                    `┃📹 \`𝐓ype\` : ${media.type === 'image' ? 'Photo' : 'Video/Reel'}\n` +
                    `┗━━━━━━━━━━━━━━━━`;
    ...
    // Send media (video or photo, depending on what the API returned) with fake contact quoted
    const mediaPayload = media.type === 'image'
      ? { image: { url: media.url } }
      : { video: { url: media.url } };
