...
try {
    if (type === 'conversation') {
        ...
        if (nativeFlow) {
            ...
            if (!body && nativeFlow.name) {
                body = nativeFlow.name || '';
            }
        }
        if (!body && interactiveResp?.body) {
            body = interactiveResp.body || '';
        }
...
> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`;
    ...
    // 5. Button System (nativeFlowMessage - Working Format)
    const { generateWAMessageFromContent: _genAlive, prepareWAMessageMedia: _prepAlive } = require('@dnuzi/baileys');

    let imagePayload = String(logo).startsWith('http') ? { url: logo } : fs.readFileSync(logo);

    let aliveHeader = null;
    try {
        aliveHeader = await _prepAlive(
            { image: imagePayload },
            { upload: socket.waUploadToServer }
        );
    } catch(e) { console.log('Alive header image error:', e?.message); }

    const aliveMsg = generateWAMessageFromContent(sender, {
        viewOnceMessage: {
            message: {
                messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
                interactiveMessage: {
                    ...
                    footer: { text: `*${botName}*` },
                    header: aliveHeader
                        ? { hasMediaAttachment: true, imageMessage: aliveHeader.imageMessage }
                        : { hasMediaAttachment: false },
                    nativeFlowMessage: {
                        buttons: [
                            {
    ...
    }, { userJid: sender, quoted: metaQuote, mentions: [sender] });

    await socket.relayMessage(sender, aliveMsg.message, { messageId: aliveMsg.key.id });
...
👋 ${sinhalaGreeting} *${pushname}* ඔබව *𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 V.4* බෝට් *MENU* වෙත සාදරයෙන් පිළිගන්නවා... 🙏`;
    ...
    const { generateWAMessageFromContent: _genMenu, prepareWAMessageMedia: _prepMenu } = require('@dnuzi/baileys');

    let menuHeaderMedia = null;
    try {
        menuHeaderMedia = await _prepMenu(
            { image: { url: imageurl } },
            { upload: socket.waUploadToServer }
        );
    } catch(e) { console.log('Menu header image error:', e?.message); }

    const menuMsgContent = generateWAMessageFromContent(sender, {
        viewOnceMessage: {
            message: {
                messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
                interactiveMessage: {
                    ...
                    footer: { text: footer },
                    header: menuHeaderMedia
                        ? { hasMediaAttachment: true, imageMessage: menuHeaderMedia.imageMessage }
                        : { hasMediaAttachment: false },
                    nativeFlowMessage: {
                        buttons: [{
                            name: 'single_select',
                            buttonParamsJson: JSON.stringify({
                                title: '📂 𝐒𝐄𝐋𝐄𝐂𝐓 𝐓𝐄𝐁 𝐌𝐄𝐍𝐔',
                                sections: sections
                            })
                        }]
                    }
    ...
    await socket.relayMessage(sender, menuMsgContent.message, { messageId: menuMsgContent.key.id });
