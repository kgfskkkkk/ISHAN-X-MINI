...
case 'mp4': {
    try {
        ...
        // ✅ URL detect කළොත් — Download Format menu එක show කරන්න
        const isYtUrl = /^(https?:\/\/)?(www\.)?(youtube\.com\/(watch\?v=|shorts\/)|youtu\.be\/)[\w\-]+/.test(q);

        // Helper function to build and send the interactive format menu
        const sendYtFormatMenu = async (videoInfo, videoUrl) => {
            const { generateWAMessageFromContent: genWA, prepareWAMessageMedia: prepMedia } = require('@dnuzi/baileys');
            const ownerdata2 = (await axios.get(
                'https://raw.githubusercontent.com/Nethmika-LK/Shala-MD-Database/refs/heads/main/Ditelse.json'
            )).data;
            const menuFooter = ownerdata2.footer || config.BOT_FOOTER;
            ...
            const menuCaption =
                ...
                (videoInfo.views ? `*┃➤ 👀 Views    :* ${videoInfo.views}\n` : '') +
                `*┃➤ 🔗 Link     :* ${videoUrl}\n` +
                `*┗━━━━━━━━━━━━━━━━❖●►*`;
            ...
            if (videoInfo.thumbnail) {
                try {
                    headerMedia = await prepMedia(
                        { image: { url: videoInfo.thumbnail } },
                        { upload: socket.waUploadToServer }
                    );
                } catch (e) { console.error('Thumbnail load error:', e.message); }
            }

            const interactiveHeader = headerMedia
                ? { hasMediaAttachment: true, imageMessage: headerMedia.imageMessage }
                : { hasMediaAttachment: false };

            const interMsg = genWA(sender, {
                viewOnceMessage: {
                    message: {
                        messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
                        interactiveMessage: {
                            body: { text: captainText },
                            footer: { text: menuFooter },
                                        ...
                                        sections: [
                                            {
                                                title: '🎥 Video Format',
                                                rows: [
                                                    { header: '360p', title: '360p Quality Video', description: '360p quality download', id: `${prefix}down_360 ${videoUrl}` },
                                                    { header: '480p', title: '480p Quality Video', description: '480p quality download', id: `${prefix}down_480 ${videoUrl}` },
                                                    { header: '720p', title: '720p Quality Video', description: '720p quality download', id: `${prefix}down_720 ${videoUrl}` },
                                                    { header: '1080p', title: '1080p Quality Video', description: '1080p quality download', id: `${prefix}down_1080 ${videoUrl}` }
                                                ]
                                            },
                                            {
                                                title: '📂 Document Format',
                                                rows: [
                                                    { header: '360p', title: '360p Quality Video', description: '360p quality download', id: `${prefix}down_360d ${videoUrl}` },
                                                    { header: '480p', title: '480p Quality Video', description: '480p quality download', id: `${prefix}down_480d ${videoUrl}` },
                                                    { header: '720p', title: '720p Quality Video', description: '720p quality download', id: `${prefix}down_720d ${videoUrl}` },
                                                    { header: '1080p', title: '1080p Quality Video', description: '1080p quality download', id: `${prefix}down_1080d ${videoUrl}` }
                                                ]
                                            }
                                        ]
                                    })
            ...
            await socket.relayMessage(sender, interMsg.message, { messageId: interMsg.key.id });
        };

        if (isYtUrl) {
            await socket.sendMessage(sender, { react: { text: '🎥', key: msg.key } });
            // URL දුන්නාහම video info ලබාගෙන format menu show කරන්න
            let videoInfo = { title: 'YouTube Video', thumbnail: null, channel: null, duration: null, views: null };
            try {
                const searchByUrl = await ytsv(q);
                const firstVid = searchByUrl.videos?.[0] || null;
                if (firstVid) {
                    videoInfo = {
                        title: firstVid.title || 'YouTube Video',
                        thumbnail: firstVid.thumbnail || null,
                        channel: firstVid.author?.name || null,
                        duration: firstVid.timestamp || null,
                        views: firstVid.views ? firstVid.views.toLocaleString() : null
                    };
                }
            } catch (e) { console.error('yt-search for URL info failed:', e.message); }
            await sendYtFormatMenu(videoInfo, q);
            break;
        }
        ...
        const videoInfo2 = {
            ...
            views: video.views ? video.views.toLocaleString() : null
        };

        await sendYtFormatMenu(videoInfo2, video.url);
