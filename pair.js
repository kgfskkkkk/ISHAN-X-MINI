...
// -------- BUTTON MODE HELPER --------
// BUTTON_MODE on  => buttons show කරනවා (default)
// BUTTON_MODE off => buttons remove කරලා numbers reply දෙනවා

async function isButtonModeOn(number) {
  try {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    // Default on if not set
    return (cfg.BUTTON_MODE !== undefined ? cfg.BUTTON_MODE : config.BUTTON_MODE) !== 'off';
  } catch (e) { return true; }
}

// Convert buttons array to a numbered text list
function buttonsToNumberText(buttons) {
  if (!buttons || !buttons.length) return '';
  return '\n\n' + buttons.map((b, i) => {
    const label = b.buttonText?.displayText || b.name || `Option ${i+1}`;
    return `*${i+1}.* ${label}`;
  }).join('\n');
}

// Convert interactive buttons (interactiveMessage format) to numbered text
function interactiveButtonsToNumberText(buttons) {
  if (!buttons || !buttons.length) return '';
  return '\n\n' + buttons.map((b, i) => {
    try {
      const params = JSON.parse(b.buttonParamsJson || '{}');
      const label = params.display_text || params.displayText || `Option ${i+1}`;
      return `*${i+1}.* ${label}`;
    } catch(e) {
      return `*${i+1}.* Option ${i+1}`;
    }
  }).join('\n');
}

// Smart send: if BUTTON_MODE off, strip buttons and send as text/image with numbered list
async function smartSend(socket, sender, payload, options, number) {
  const btnOn = await isButtonModeOn(number);
  if (btnOn) {
    return await socket.sendMessage(sender, payload, options);
  }
  // Button mode OFF - remove buttons and add numbered list to caption/text
  const p = { ...payload };
  let numberedList = '';

  if (p.buttons && p.buttons.length) {
    numberedList = buttonsToNumberText(p.buttons);
    delete p.buttons;
  }
  delete p.headerType;
  delete p.footer;
  ...
  if (p.interactiveMessage) {
    try {
      const btns = p.interactiveMessage.nativeFlowMessage?.buttons ||
                   p.interactiveMessage.body?.buttons || [];
      numberedList = interactiveButtonsToNumberText(btns);
      const bodyText = p.interactiveMessage.body?.text ||
                       p.interactiveMessage.header?.imageMessage?.caption || '';
      const headerImg = p.interactiveMessage.header?.imageMessage?.url ||
                        p.interactiveMessage.header?.imageMessage?.jpegThumbnail;
      if (headerImg) {
        return await socket.sendMessage(sender, {
          image: { url: headerImg },
          caption: bodyText + numberedList
        }, options);
      }
      return await socket.sendMessage(sender, {
        text: bodyText + numberedList
      }, options);
    } catch(e) {}
  ...
  if (p.caption !== undefined) {
    p.caption = (p.caption || '') + numberedList;
  } else if (p.text !== undefined) {
    p.text = (p.text || '') + numberedList;
  }

  return await socket.sendMessage(sender, p, options);
}
// -------- END BUTTON MODE HELPER --------
...
async function launchMovieBrowser() {
  return puppeteer.launch({
    ...
    args: [
      ...
      "--no-first-run",
      "--no-zygote",
      "--single-process",             // lower memory footprint on constrained hosts
...
*│➣ ʙᴜᴛᴛᴏɴ ᴍᴏᴅᴇ* ${currentConfig.BUTTON_MODE || 'on'}
*✦╰ᴀᴜᴛᴏ ᴛʏᴘɪɴɢ* ${currentConfig.AUTO_RECORDING || 'false'}
*╰─╯*
    `.trim();

    // 6. Send the Message
    await smartSend(socket, sender, {
      headerType: 1,
      viewOnce: true,
...
case 'buttonmode': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_BTNMODE1" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
      return await socket.sendMessage(sender, { text: '❌ Permission denied. Only the session owner or bot owner can change button mode.' }, { quoted: shonux });
    }
    ...
    if (q === 'on' || q === 'off') {
      const userConfig = await loadUserConfigFromMongo(sanitized) || {};
      userConfig.BUTTON_MODE = q;
      await setUserConfigInMongo(sanitized, userConfig);

      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_BTNMODE2" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };

      const statusMsg = q === 'on'
        ? `✅ *Button Mode ENABLED*\n\nBot responses will now show interactive buttons.`
        : `✅ *Button Mode DISABLED*\n\nBot responses will now use numbered text list instead of buttons.\n\n_Example:_\n*1.* 📄 MAIN MENU\n*2.* 📡 BOT INFO`;

      await socket.sendMessage(sender, { text: statusMsg }, { quoted: shonux });
    } else {
      const userConfig = await loadUserConfigFromMongo(sanitized) || {};
      const currentMode = userConfig.BUTTON_MODE || config.BUTTON_MODE || 'on';
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_BTNMODE3" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
      await socket.sendMessage(sender, {
        text: `🔘 *Button Mode Settings*\n\nCurrent Status: *${currentMode.toUpperCase()}*\n\nUsage:\n• \`.buttonmode on\` - Enable buttons\n• \`.buttonmode off\` - Disable buttons (use numbers instead)`
      }, { quoted: shonux });
    }
  } catch (e) {
    console.error('Buttonmode error:', e);
    await socket.sendMessage(sender, { text: '❌ Error updating button mode!' }, { quoted: msg });
  }
  break;
}
...
case 'mainmenu': {
  try {
    ...
    await smartSend(socket, sender, { image: { url: imageurl }, caption: menuc, footer: footer, buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "↩️ MENU COMMAND" }, type: 1 }], headerType: 4 }, { quoted: msg }, number);
