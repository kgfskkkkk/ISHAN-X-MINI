...
// 🎬 ArslanMD Style Direct Video Download
case 'video2': {
    try {
        const ytsv2 = require('yt-search');

        const q2 = args.join(' ').trim();
        if (!q2) return await socket.sendMessage(sender, { text: '*📛 Example: .video2 pasoori*' }, { quoted: msg });

        await socket.sendMessage(sender, { react: { text: '🎬', key: msg.key } });

        const search2 = await ytsv2(q2);
        if (!search2.videos.length) return await socket.sendMessage(sender, { text: '*❌ No video found*' }, { quoted: msg });
        ...
        console.log('video2 API raw response:', JSON.stringify(d2));

        // Try multiple possible response shapes since unofficial APIs change format often
        const r2 = d2?.result || d2?.data || d2;
        videoUrl2 =
            r2?.download?.url ||
            r2?.download_url ||
            r2?.downloadUrl ||
            ...
            r2?.video_url ||
            r2?.link ||
            (Array.isArray(r2?.medias) && (r2.medias.find(m => m.type === 'video')?.url || r2.medias[0]?.url)) ||
            null;

        title2 = r2?.metadata?.title || r2?.title || vid2.title;

        if (!videoUrl2) {
            console.error('video2: could not extract video url from response:', JSON.stringify(d2));
            return await socket.sendMessage(sender, { text: `*❌ Video API failed*\n\n_API response:_\n${JSON.stringify(d2).slice(0, 500)}` }, { quoted: msg });
        }
        ...
        console.error('video2 Error:', err?.response?.data || err.message || err);
        const errMsg = err?.response?.data ? JSON.stringify(err.response.data).slice(0, 300) : (err.message || 'Unknown error');
        await socket.sendMessage(sender, { text: `*❌ Video error*\n_${errMsg}_` }, { quoted: msg });
    }
    break;
}
