...
// ==================== SONG (ytmp3) DOWNLOAD HELPERS ====================
// Centralizes what ytaa / ytaap / ytad all need: a clean YouTube URL pulled
// out of whatever the user/button sent, a primary downloader call with a
// timeout (so a hung 3rd-party API doesn't just sit there), one retry, and a
// fallback API so a single dead provider doesn't take song downloads out
// entirely. Previously all 3 commands swallowed the *real* error into a
// generic "🚩 Song Dl Error" with no detail sent to the user, which made it
// impossible to tell whether the URL was bad, the API was down, rate
// limited, or returned an unexpected shape.

const YT_URL_REGEX = /(?:https?:\/\/)?(?:www\.)?(?:music\.)?(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/;

// Pulls a clean https://youtu.be/<id> link out of free-form input. Handles:
// - a raw URL typed by the user (possibly with extra query junk like ?si=)
// - the "url±thumb±title" payload that the ytad button sends
// - already-clean URLs
function extractYoutubeUrl(input) {
  if (!input) return null;
  const firstPart = String(input).split('±')[0].trim();
  const match = firstPart.match(YT_URL_REGEX);
...
// Fallback provider used only if sadaslk-dlcore fails or is unreachable.
// Keeps song downloads working even if that one package/API goes down.
async function ytmp3Fallback(youtubeUrl) {
  const { data } = await axios.get('https://api.giftedtech.web.id/api/download/ytmp3', {
    params: { apikey: 'gifted', url: youtubeUrl },
    timeout: 25000
  });
  const url = data?.result?.download_url || data?.result?.url || data?.url;
  if (!url) throw new Error('Fallback API did not return a download URL');
  return { url, title: data?.result?.title };
}

// Single entry point all 3 commands (ytaa/ytaap/ytad) should call.
// Throws a clear, user-facing Error message on failure instead of a vague one.
async function fetchSongAudio(rawInput) {
  const youtubeUrl = extractYoutubeUrl(rawInput);
  if (!youtubeUrl) {
    throw new Error('Invalid or missing YouTube URL/link. Use .song <name or link> first, then pick a format.');
  }

  const dlcore = require('sadaslk-dlcore');
  let lastErr;

  // Try primary provider, 1 retry on transient failure.
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      const result = await withTimeout(dlcore.ytmp3(youtubeUrl), 30000, 'sadaslk-dlcore');
      if (result && result.url) return result;
      lastErr = new Error('sadaslk-dlcore returned no download URL (API may have changed its response format or the video is restricted)');
    } catch (e) {
      lastErr = e;
    }
  ...
  console.error('[song] primary provider failed, trying fallback:', lastErr?.message);

  // Primary exhausted -> try fallback provider once before giving up.
  try {
    const fallbackResult = await withTimeout(ytmp3Fallback(youtubeUrl), 30000, 'fallback API');
    return fallbackResult;
  } catch (fallbackErr) {
    console.error('[song] fallback provider also failed:', fallbackErr.message);
    // Surface the most informative message we have.
    const reason = lastErr?.message || fallbackErr.message || 'unknown error';
    throw new Error(`Could not fetch audio (${reason}). The video may be private/region-locked, or the download API is temporarily down.`);
  }
}
// ==================== END SONG DOWNLOAD HELPERS ====================
...
try {
    ...
    if (!command) {
      ...
      if (moviePendingSearch[sender] && Array.isArray(moviePendingSearch[sender].results) && !isNaN(replyNum) && replyNum > 0 && replyNum <= moviePendingSearch[sender].results.length) {
        try {
      ...
      if (moviePendingQuality[sender] && moviePendingQuality[sender].movie && Array.isArray(moviePendingQuality[sender].movie.downloadLinks) && !isNaN(replyNum) && replyNum > 0 && replyNum <= moviePendingQuality[sender].movie.downloadLinks.length) {
        try {
          await socket.sendMessage(from, { react: { text: "✅", key: msg.key } });
          const index = replyNum - 1;
...
case 'ytaa': {
    try {
        ...
        const result = await fetchSongAudio(q);
        ...
        await socket.sendMessage(sender, { text: `*🚩 Song Dl Error*\n${e.message}` }, { quoted: msg });
    }
    break;
}
case 'ytaap': {
    ...
    try {
        ...
        mp3File = path.join(os.tmpdir(), `ytaap_${Date.now()}.mp3`);
        oggFile = path.join(os.tmpdir(), `ytaap_${Date.now()}.ogg`);

        const dl = await axios.get(result.url, { responseType: 'arraybuffer', timeout: 30000 });
        ...
        // Always clean up, even if a step above threw partway through.
        try { if (mp3File && fs.existsSync(mp3File)) fs.unlinkSync(mp3File); } catch (e) {}
        try { if (oggFile && fs.existsSync(oggFile)) fs.unlinkSync(oggFile); } catch (e) {}
    }
    break;
}
case 'ytad': {
    try {
        ...
        const parts = q.split('±');
        const url = parts[0];
        const title = (parts[2] || 'Song').replace(/[\\/:*?"<>|]/g, '').trim() || 'Song';

        const result = await fetchSongAudio(url);

        const match = url.match(YT_URL_REGEX);
        let resizedBotImg;
        if (match && match[1]) {
            // Thumbnail is cosmetic — don't let a failed thumbnail fetch kill the whole download.
            try {
                const thumbUrl = `https://i.ytimg.com/vi/${match[1]}/hqdefault.jpg`;
                const res = await axios.get(thumbUrl, { responseType: 'arraybuffer', timeout: 15000 });
                resizedBotImg = await sharp(Buffer.from(res.data)).resize(200, 200).toBuffer();
            } catch (thumbErr) {
                console.error('ytad thumbnail fetch failed (non-fatal):', thumbErr.message);
            }
        }
