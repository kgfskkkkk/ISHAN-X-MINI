...
// ===== BUTTON MODE HELPER =====
// buttonMode=true  → buttons show කරනවා (normal)
// buttonMode=false → buttons අයින් කර text number list show කරනවා
async function sendMsgSmart(socket, to, payload, options, buttonMode) {
  if (buttonMode) {
    return socket.sendMessage(to, payload, options);
  }
  // Button mode off: remove buttons, add number list to caption/text
  const noBtn = Object.assign({}, payload);
  if (noBtn.buttons && Array.isArray(noBtn.buttons)) {
    const numList = noBtn.buttons.map((b, i) => `*${i+1}️⃣* - ${b.buttonText?.displayText || b.name || ''}`).join('\n');
    const field = noBtn.caption !== undefined ? 'caption' : 'text';
    if (noBtn[field]) {
      noBtn[field] = noBtn[field] + `\n\n*📋 𝙍𝙀𝙋𝙇𝙔 𝙒𝙄𝙏𝙃 𝙉𝙐𝙈𝘽𝙀𝙍:*\n` + numList;
    }
    delete noBtn.buttons;
    delete noBtn.headerType;
    delete noBtn.footer;
  }
  return socket.sendMessage(to, noBtn, options);
}
...
try {
    ...
    try {
      ...
      // Button Mode: default 'true' (use buttons). 'false' = number reply system
      const buttonMode = (userConfig.BUTTON_MODE !== undefined ? userConfig.BUTTON_MODE : config.BUTTON_MODE) !== 'false';
...
// ========== END WORK TYPE RESTRICTIONS ==========
      ...
      switch (command) {
        ...
        // ===== BUTTON MODE ON =====
        case 'buttonon': {
          if (!isOwner) return await socket.sendMessage(sender, { text: '❌ *මෙම command owner ට පමණයි!*' }, { quoted: msg });
          const sanitizedBtn = (number || '').replace(/[^0-9]/g, '');
          let cfgBtn = await loadUserConfigFromMongo(sanitizedBtn) || {};
          cfgBtn.BUTTON_MODE = 'true';
          await setUserConfigInMongo(sanitizedBtn, cfgBtn);
          return await socket.sendMessage(sender, {
            text: `✅ *Button Mode ON කරන ලදී!*\n\n🔘 දැන් bot ෙගේ replies හා menus ෙකෙදිම buttons show වෙනවා.\n\n> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`
          }, { quoted: msg });
        }
        ...
        case 'buttonoff': {
          if (!isOwner) return await socket.sendMessage(sender, { text: '❌ *මෙම command owner ට පමණයි!*' }, { quoted: msg });
          const sanitizedBtn2 = (number || '').replace(/[^0-9]/g, '');
          let cfgBtn2 = await loadUserConfigFromMongo(sanitizedBtn2) || {};
          cfgBtn2.BUTTON_MODE = 'false';
          await setUserConfigInMongo(sanitizedBtn2, cfgBtn2);
          return await socket.sendMessage(sender, {
            text: `✅ *Button Mode OFF කරන ලදී!*\n\n🔢 දැන් bot ෙගේ replies හා menus ෙකෙදිම buttons අයින් වෙලා number reply system use කරනවා.\n\n> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`
          }, { quoted: msg });
        }
          case 'ts': {
    const axios = require('axios');
...
> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`;
    ...
    if (buttonMode) {
      // Button Mode ON - buttons show කරනවා
      await socket.sendMessage(sender, {
        image: imagePayload,
        caption: text,
        footer: `*${botName}*`,
      ...
      // Button Mode OFF - number reply system
      const aliveNumberText = text + `\n\n*📋 𝙍𝙀𝙋𝙇𝙔 𝙒𝙄𝙏𝙃 𝙉𝙐𝙈𝘽𝙀𝙍:*\n*1️⃣* - 📑 Bot Menu\n*2️⃣* - 📶 Speed Test\n*3️⃣* - 👩‍💻 Help`;
      await socket.sendMessage(sender, {
        image: imagePayload,
        caption: aliveNumberText,
        footer: `*${botName}*`,
        mentions: [sender]
      }, { quoted: metaQuote });
    }
...
👋 ${sinhalaGreeting} *${pushname}* ඔබව *𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 V.4* බෝට් *MENU* වෙත සාදරයෙන් පිළිගන්නවා... 🙏`;
    ...
    if (buttonMode) {
      ...
      // Button Mode OFF - numbered text menu
      const menuNumberText = menuMessage + `\n\n*📋 𝙈𝙀𝙉𝙐 𝙇𝙄𝙎𝙏:*\n*1️⃣* - 🏡 Main Menu (.mainmenu)\n*2️⃣* - 🧑‍💻 Owner Menu (.ownermenu)\n*3️⃣* - 💑 Group Menu (.groupmenu)\n*4️⃣* - 📰 News Menu (.newsmenu)\n*5️⃣* - 🎴 Sticker Menu (.stickermenu)\n*6️⃣* - ⚙️ Settings (.settings)\n*7️⃣* - 📥 Download Menu (.downloadmenu)\n*8️⃣* - 🎨 Tool Menu (.toolmenu)`;
      await socket.sendMessage(sender, {
        image: { url: imageurl },
        caption: menuNumberText,
        footer: footer
      }, { quoted: msg });
    }
...
case 'mainmenu': {
  try {
    ...
    await sendMsgSmart(socket, sender, { image: { url: imageurl }, caption: menuc, footer: footer, buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "↩️ MENU COMMAND" }, type: 1 }], headerType: 4 }, { quoted: msg }, buttonMode);
...
`.trim();
    ...
    if (buttonMode) {
      ...
      await socket.sendMessage(sender, {
        ...
        caption: text + `\n\n*📋 𝙈𝙀𝙉𝙐 𝙊𝙋𝙏𝙄𝙊𝙉𝙎:*\n` + buttons.map((b,i)=>`*${i+1}️⃣* - ${b.buttonText?.displayText||''}`).join('\n'),
        footer: "〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰"
      }, { quoted: shonux });
    }
...
END:VCARD` 
        ...
        await socket.sendMessage(msg.key.remoteJid, {
            image: { url: ppUrl },
            caption: buttonMode ? caption : caption + `\n\n*📋 𝙊𝙋𝙏𝙄𝙊𝙉𝙎:*\n` + buttons.map((b,i)=>`*${i+1}️⃣* - ${b.buttonText?.displayText||''}`).join('\n'),
            footer: `Power by ${botName}`,
            ...(buttonMode ? { buttons: buttons, headerType: 4 } : {}),
            mentions: [targetUser]
        }, { quoted: metaQuote });
...
END:VCARD` } }
        ...
        const buttonMessage = buttonMode ? {
...
case 'apks': {
    try {
        ...
        await socket.sendMessage(sender, {
            image: { url: apk.icon },
            caption: buttonMode ? caption : caption + `\n\n*📋 𝙊𝙋𝙏𝙄𝙊𝙉𝙎:*\n*1️⃣* - 📂 Download (.apkdl ${apk.package})\n*2️⃣* - 📑 Details (.apkdtl ${apk.package})`,
            footer: footer,
            ...(buttonMode ? { buttons: buttons, headerType: 4 } : {})
        }, { quoted: msg });
