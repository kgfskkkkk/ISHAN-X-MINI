...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // Unwrap viewOnceMessageV2 / viewOnceMessageV2Extension so that button
    // responses to interactive messages sent inside viewOnceMessage are handled.
    const _typeAfterEphemeral = getContentType(msg.message);
    if (_typeAfterEphemeral === 'viewOnceMessageV2') {
        msg.message = msg.message.viewOnceMessageV2?.message || msg.message;
    } else if (_typeAfterEphemeral === 'viewOnceMessageV2Extension') {
        msg.message = msg.message.viewOnceMessageV2Extension?.message || msg.message;
    }

    const effectiveType = getContentType(msg.message);
...
try {
    ...
    } else if (effectiveType === 'extendedTextMessage') {
        body = msg.message.extendedTextMessage?.text || '';
    } else if (effectiveType === 'imageMessage') {
        body = msg.message.imageMessage?.caption || '';
    } else if (effectiveType === 'videoMessage') {
        body = msg.message.videoMessage?.caption || '';
    } else if (effectiveType === 'buttonsResponseMessage') {
        body = msg.message.buttonsResponseMessage?.selectedButtonId || '';
    } else if (effectiveType === 'listResponseMessage') {
        body = msg.message.listResponseMessage?.singleSelectReply?.selectedRowId || '';
    } else if (effectiveType === 'templateButtonReplyMessage') {
        body = msg.message.templateButtonReplyMessage?.selectedId || '';
    } else if (effectiveType === 'interactiveResponseMessage') {
    ...
    } else if (effectiveType === 'viewOnceMessage') {
