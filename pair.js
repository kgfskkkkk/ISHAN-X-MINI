...
END:VCARD`
    ...
    // NOTE: different deploys of this API have returned slightly different
    // JSON shapes, so we defensively check several known possibilities.
    let data = null;
    if (apiRes) {
      const pickFromArray = (arr) => {
        if (!Array.isArray(arr) || arr.length === 0) return null;
        const media = arr.find(item => item?.type === 'video' || item?.url) || arr[0];
        return media?.url || media?.download_url || media?.downloadUrl || null;
      };
      ...
      const candidateUrl =
        ...
        apiRes.download_url ||
        apiRes?.result?.url ||
        apiRes?.result?.video_url ||
        apiRes?.result?.download_url ||
        apiRes?.data?.url ||
        apiRes?.data?.video_url ||
        pickFromArray(apiRes.data) ||
        pickFromArray(apiRes.result) ||
        pickFromArray(apiRes.medias) ||
        pickFromArray(apiRes.media) ||
        null;

      if (candidateUrl) {
        data = { status: true, downloadUrl: candidateUrl };
      } else {
        // Helps diagnose if NN Tech ever changes their response format again.
        console.log("IG API unexpected response:", JSON.stringify(apiRes));
      }
    }
