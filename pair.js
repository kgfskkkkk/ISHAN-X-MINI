    ...
    const settingOptions = {
      ...
      paramsJson: JSON.stringify({
        ...
        sections: [
          ...
          {
            title: '🍷 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 ʙᴜᴛᴛᴏɴ ᴍᴏᴅᴇ',
            rows: [
              { title: '❄ ➤ 𝐁𝐮𝐭𝐭𝐨𝐧 𝐌𝐨𝐝𝐞 ▸ 𝐎𝐍', description: 'Use interactive buttons', id: `${prefix}setbutton true` },
              { title: '❄ ➤ 𝐁𝐮𝐭𝐭𝐨𝐧 𝐌𝐨𝐝𝐞 ▸ 𝐎𝐅𝐅', description: 'Use plain text mode', id: `${prefix}setbutton false` },
            ],
          },
        ],
      }),
...
*│➣ ʙᴜᴛᴛᴏɴ ᴍᴏᴅᴇ* ${currentConfig.BUTTON || config.BUTTON || 'true'}
*╰─╯*
    `.trim();

    // 6. Send the Message
    const buttonMode = currentConfig.BUTTON || config.BUTTON || 'true';
    if (buttonMode === 'true') {
      await socket.sendMessage(sender, {
        headerType: 1,
        viewOnce: true,
...
➤ \`${prefix}resetconfig\` — Reset all settings

🍷 𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐁𝐲 ${config.OWNER_NAME || 'Bot Owner'}`,
      }, { quoted: msg });
    }
...
case 'button': {
  ...
  try {
    ...
    if (senderNum !== sanitized && senderNum !== ownerNum) {
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_SETBUTTON1" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
    ...
    if (val !== 'true' && val !== 'false') {
      const shonux = {
        key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_SETBUTTON2" },
        message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
      };
      return await socket.sendMessage(sender, { text: '*Usage:* .setbutton true / false\n\n• *true* — Use interactive buttons\n• *false* — Use plain text mode' }, { quoted: shonux });
    }

    const userConfig = await loadUserConfigFromMongo(sanitized) || {};
    userConfig.BUTTON = val;
    await setUserConfigInMongo(sanitized, userConfig);

    const shonux = {
      key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_SETBUTTON3" },
      message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
    };
    await socket.sendMessage(sender, { text: `✅ *Button Mode updated to:* _${val}_\n\n${val === 'true' ? '🔘 Interactive buttons are now ON' : '📝 Plain text mode is now ON'}` }, { quoted: shonux });
  } catch (e) {
    console.error('Setbutton error:', e);
    const shonux = {
      key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_SETBUTTON4" },
      message: { contactMessage: { displayName: BOT_NAME_FANCY, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${BOT_NAME_FANCY};;;;\nFN:${BOT_NAME_FANCY}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
    };
    await socket.sendMessage(sender, { text: "*❌ Error updating button mode!*" }, { quoted: shonux });
  }
  break;
}
...
case 'gpt': {
  try {
    ...
    const aiSanitized = (number || '').replace(/[^0-9]/g, '');
    const aiCfg = await loadUserConfigFromMongo(aiSanitized) || {};
    const aiButtonMode = aiCfg.BUTTON || config.BUTTON || 'true';

    if (!q) {
      if (aiButtonMode === 'true') {
        await socket.sendMessage(sender, { 
        ...
        await socket.sendMessage(sender, { text: `* 𝙿𝚕𝚎𝚊𝚜𝚎 𝙿𝚛𝚘𝚟𝚒𝚍𝚎 𝙰 𝚀𝚞𝚎𝚜𝚝𝚒𝚘𝚗 !*\n\nType: ${config.PREFIX}ai <your question>` }, { quoted: msg });
      }
      return;
    }
    ...
    if ((await (async () => { const c = await loadUserConfigFromMongo((number||'').replace(/[^0-9]/g,'')); return (c&&c.BUTTON)||config.BUTTON||'true'; })()) === 'true') {
      await socket.sendMessage(sender, { 
      ...
      await socket.sendMessage(sender, { text: '*𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 𝙰𝙿𝙸 𝙴𝚛𝚛𝚘𝚛 𝚃𝚛𝚢 𝙰𝚐𝚊𝚒𝚗 𝙻𝚊𝚝𝚎𝚛 !*' }, { quoted: msg });
    }
  }
  break;
}
...
END:VCARD`
        ...
        const xvSanitized = (number || '').replace(/[^0-9]/g, '');
        const xvUserCfg = await loadUserConfigFromMongo(xvSanitized) || {};
        const xvButtonMode = xvUserCfg.BUTTON || config.BUTTON || 'true';

        if (!query) {
            if (xvButtonMode === 'true') {
              return await socket.sendMessage(sender, {
...
> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰*`;
    ...
    const aliveSanitized = (number || '').replace(/[^0-9]/g, '');
    const aliveUserCfg = await loadUserConfigFromMongo(aliveSanitized) || {};
    const aliveButtonMode = aliveUserCfg.BUTTON || config.BUTTON || 'true';

    if (aliveButtonMode === 'true') {
      await socket.sendMessage(sender, {
        image: imagePayload,
        caption: text,
        footer: `*${botName}*`,
      ...
      await socket.sendMessage(sender, {
        image: imagePayload,
        caption: `${text}\n\n*📋 Commands:* ${config.PREFIX}menu | ${config.PREFIX}ping | ${config.PREFIX}help`,
        footer: `*${botName}*`,
        mentions: [sender]
      }, { quoted: metaQuote });
    }
...
END:VCARD` 
        ...
        // --- Check Button Mode ---
        const ppSanitized = (number || '').replace(/[^0-9]/g, '');
        const ppUserCfg = await loadUserConfigFromMongo(ppSanitized) || {};
        const ppButtonMode = ppUserCfg.BUTTON || config.BUTTON || 'true';

        // --- SEND MESSAGE ---
        if (ppButtonMode === 'true') {
          ...
          await socket.sendMessage(msg.key.remoteJid, {
              image: { url: ppUrl },
              caption: `${caption}\n\n*📋 Commands:* ${config.PREFIX || '.'}menu | ${config.PREFIX || '.'}alive`,
              mentions: [targetUser]
          }, { quoted: metaQuote });
        }
